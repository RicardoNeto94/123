...
(async function init(){
  ...
  const rerender=()=>{ const sel=getActiveFilters(); const filtered=filterDishes(data,sel,search.value); renderGrid(grid,filtered,sel);
addSafeChecks(); updateMeta(filtered.length,sel); };
...
function addSafeChecks(){
  try{
    const sel = (typeof selectedAllergens!=='undefined' && selectedAllergens) ? selectedAllergens : [];
    if(!sel.length) return;
    document.querySelectorAll('.card').forEach(card=>{
      const data = card.__data || null;
      const allergens = data && Array.isArray(data.allergens) ? data.allergens : [];
      const pass = sel.every(c => !allergens.includes(c));
      if(pass){
        const exists = card.querySelector('.safe-check'); 
        if(!exists){
